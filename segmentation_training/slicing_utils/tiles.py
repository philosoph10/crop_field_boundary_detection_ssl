...
class ImageSlicer:
    ...
    def __init__(
        ...
        for y in range(
            0,
            self.image_height + self.margin_top + self.margin_bottom - self.tile_size[0] + 1,
            self.tile_step[0],
        ):
            for x in range(
                0,
                self.image_width + self.margin_left + self.margin_right - self.tile_size[1] + 1,
                self.tile_step[1],
            ):
    ...
    def iter_split(
        ...
        for _, crop_coords in zip(self.crops, self.bbox_crops):
