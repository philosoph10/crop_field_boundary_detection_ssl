...
def infer(
    ...
    image_tensor = get_val_transform(image.shape[:2])(image=image)["image"]
    ...
    with torch.no_grad():
        ...
        if mode == "multiclass":
            ...
            if not return_conf:
                mask = (mask > 0.5).to(torch.int)[0, 0]
            else:
                mask = mask[0, 0]
        else:
...
def sliced_inference(
    model: torch.nn.Module, image: np.ndarray, slicer: ImageSlicer, conf: float = 0.5, mode: str = "binary"
) -> np.ndarray:
    """
    Run sliced inference on an image.
    Supported modes: {`binary`}
    Output format:
     - binary: a binary mask with values 0 or 255
    ...
    for tile, (x, y, tile_width, tile_height) in slicer.iter_split(image):
        mask = infer(model, tile, mode=mode, return_conf=True)
